 ...
 * @returns 
 */
const percentDiff = (final, initial) => {
	const diff = Math.round((final - initial) / initial * 100)
...
const renderMatchDetails = (matchData) => {
	return (
		<Grid container sx={{pt: 2}}>
			...
			<Grid item xs={4} align="center" sx={{ fontWeight: "bold" }}>
            ...
            { renderSpread(matchData) }
            { renderTotal(matchData) }
		</Grid>
	)
}

const renderSpread = (matchData) => {

    // TODO - this is disgusting.. refactor
    if (matchData.postgameHomeSpread) {
        return (
            <Grid container>
                <Grid item xs={12} align="center" sx={{ fontWeight: "bold" }}>Home Team Spread</Grid>
                <Grid item xs={12} align="center">pre: {matchData.pregameHomeSpread}</Grid>
                <Grid item xs={12} align="center">post: {matchData.postgameHomeSpread} ({ percentDiff(matchData.postgameHomeSpread, matchData.pregameHomeSpread) })</Grid>
            </Grid>
        )
    } else {
        return ('')
...
const renderTotal = (matchData) => {
    // TODO - this is disgusting.. refactor
    if (matchData.postgameHomeSpread) {
        return (
            <Grid container>
                <Grid item xs={12} align="center" sx={{ fontWeight: "bold", mt: 2 }}>Total</Grid>
                <Grid item xs={12} align="center">pre: {matchData.pregameTotal}</Grid>
                <Grid item xs={12} align="center">post: {matchData.postgameTotal} ({ percentDiff(matchData.postgameTotal, matchData.pregameTotal) })</Grid>
            </Grid>
        )
    } else {
        return (
            <Grid container>
                <Grid item xs={12} align="center" sx={{ fontWeight: "bold", mt: 2 }}>Total</Grid>
                <Grid item xs={12} align="center">{matchData.pregameTotal}</Grid>
            </Grid>
        )
    }
}
